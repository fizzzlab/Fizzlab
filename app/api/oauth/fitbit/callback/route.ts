...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(`${appUrl}/connect?error=fitbit_denied`);
  }
  ...
  const redirectUri  = `${appUrl}/api/oauth/fitbit/callback`;
  ...
  if (!tokenRes.ok) {
    ...
    return NextResponse.redirect(`${appUrl}/connect?error=fitbit_token_failed`);
  }
  ...
  const response = NextResponse.redirect(`${appUrl}/connect?success=fitbit`);
